...
bool AsCommander::Init()
{
    ...
    {
        printf("GetConsoleActiveScreenBuffer failed - (%d)\n", GetLastError());
        return false;
    }
    Screen_Buffer_size = static_cast<int>(Screen_Buffer_Info.dwSize.X * static_cast<int>(Screen_Buffer_Info.dwSize.Y));
            ...
            if (ReadConsoleInput(Std_Input_Handle, input_record, 1, &record_count))
            {
                if (record_count != 0)
                {
                    if (input_record[0].EventType == KEY_EVENT && input_record[0].Event.KeyEvent.bKeyDown) {
                        ...
                        case VK_UP:
                            Left_Panel->Move_Ver_Highlight(true);
                            Need_Redraw = true;
                            break;

                        case VK_DOWN:
                            Left_Panel->Move_Ver_Highlight(false);
                            Need_Redraw = true;
                            break;
                        ...
                        case VK_RIGHT:
                            Left_Panel->Move_Hor_Highlight(true);
                            Need_Redraw = true;
                            break;

                        case VK_LEFT:
                            Left_Panel->Move_Hor_Highlight(false);
                            Need_Redraw = true;
                            break;
                        }
...
void AsCommander::Build_Menu()
{
    ...
    Add_Next_Menu_Item(index, x_pos, x_step, L" 9", L"ConfMn");
